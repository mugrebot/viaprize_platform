  ...
  describe("Functions", function () {
    ...
    it("Should allow users to vote on submissions and update their votes", async function () {
      ...
      await yourContract
        .connect(addr1)
        .change_vote(
          allSubmissionsCheck[0].submissionHash,
          allSubmissionsCheck[1].submissionHash,
    ...
    it("Should allow users to claim refunds for unfunded submissions and unused votes", async function () {
      ...
      await yourContract.connect(addr1).addFunds({ value: ethers.utils.parseEther("1") });
      await yourContract.start_voting_period(2);
      const tx = await yourContract
        .connect(addr1)
        .vote(getSubmissions[0].submissionHash, ethers.utils.parseEther("0.9"));
    ...
    it("Should not allow a user to vote with more funds than they have", async function () {
      ...
      await expect(yourContract.connect(addr1).vote(allSubmissions[0].submissionHash, ethers.utils.parseEther("0.5")))
    ...
    it("Should not allow a user to change someone else's votes", async function () {
      ...
      await expect(
        yourContract
          .connect(addr2)
          .change_vote(
            allSubmissionsState[0].submissionHash,
            allSubmissionsState[1].submissionHash,
            ethers.utils.parseEther("0.5"),
          ),
      ).to.be.revertedWithCustomError;
    });
